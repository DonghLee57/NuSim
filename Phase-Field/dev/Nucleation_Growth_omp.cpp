...
void SAVE_2D(int step, int Ngr, int z)
{
    ofstream myfile("PhaseField_" + to_string(step) + "_" + to_string(Ngr)+ "_" + to_string(z)+ ".dat");
    for (int x=1; x<Nx+1; x++) {
        for (int y=1; y<Ny+1; y++) {
        myfile << fixed << setprecision(4) << double(ETAS[Ngr][x][y][z]);
        if (y < Ny) { myfile << "  ";}
        }
        myfile << "\n";
    }
