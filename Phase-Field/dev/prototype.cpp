...
int main()
{
    ...
    for (int k = 0; k <= MAXIT; ++k) {
        ...
        if (PROB_NUC(Tsim, Va) > probDist(gen) && Xa > 0.1) {
            rx = distrX(gen);
            ry = distrY(gen);
            rz = distrZ(gen);
            ...
            if  ( Ngr < NMAX ) {
                ...
                for (int N=0; N < Ngr; N++) {
                    Ca = COUNT_AMOR(ETAS[N]);
                    if ( Nx*Ny*Nz - Ca < 100 ){
                        for (int x = 1; x < Nx + 1; ++x) {
                        for (int y = 1; y < Ny + 1; ++y) {
                        for (int z = 1; z < Nz + 1; ++z) {
                            distance = sqrt((rx - x) * (rx - x) + (ry - y) * (ry - y) + (rz - z) * (rz - z));
                            ETAS[N][x][y][z] = (distance < Rc) ? 1.0 : 0.0;
                            ETAS_TOT[x][y][z] = ETAS_TOT[x][y][z] + ETAS[N][x][y][z];
                        }}}
                    }
                    break;
                }
...
double PROB_NUC(double T, double Va)
{
    ...
    double I0 = 1.0E+52; // [m-3sec-1]
    double Ea = 1.6*PI*pow(GAMMA,3)/pow(dHf/(1-T/Tmelt),2)/3 + 5.7;     // [eV]
